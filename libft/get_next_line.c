...
/*   Updated: 2024/10/08 00:09:56 by ledio            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

int	ft_read_list(int fd, t_gnl_list **head)
{
	char	*buffer;
	int		bytes_read;
...
char	*ft_split_node(t_gnl_list *list)
{
	char	*line_part;
	char	*rest;
	int		len;
...
char	*process_newline(t_gnl_list *current, t_glfl *glfl)
{
...
char	*ft_get_line_from_list(t_gnl_list **head, t_gnl_list *current)
{
	t_glfl	glfl;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || (head == NULL && read_status == 0))
